...
metadata {
	...
	tiles {
		...
		standardTile("outlet1", "device.outlet1",canChangeIcon: false) {
			state "ono1", label: "outlet1", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "offo1", label: "outlet1", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		standardTile("outlet2", "device.outlet2",canChangeIcon: false) {
			state "ono2", label: "outlet2", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "offo2", label: "outlet2", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}


		standardTile("switch1", "device.switch1",canChangeIcon: false) {
			state "on1", label: "TV", action: "off1", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "off1", label: "TV", action: "on1", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		standardTile("switch2", "device.switch2",canChangeIcon: false) {
			state "on2", label: "Fan", action: "off2", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "off2", label: "Fan", action: "on2", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		standardTile("switch3", "device.switch3",canChangeIcon: false) {
			state "on3", label: "ASUS Cube", action: "off3", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "off3", label: "ASUS Cube", action:"on3", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		standardTile("switch4", "device.switch4",canChangeIcon: false) {
			state "on4", label: "IP Camera", action: "off4", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "off4", label: "IP Camera", action:"on4", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		standardTile("switch5", "device.switch5",canChangeIcon: false) {
			state "on5", label: "Outlet1 (sw5)", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "off5", label: "Outlet1 (sw5)", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		standardTile("switch6", "device.switch6",canChangeIcon: false) {
			state "on6", label: "Outlet2 (sw6)", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "off6", label: "Outlet2 (sw6)", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		standardTile("switch7", "device.switch7",canChangeIcon: false) {
			state "on7", label: "All On (sw7)", action: "off7", icon: "st.switches.switch.on", backgroundColor: "#79b821"
			state "off7", label: "All Off (sw7)", action: "on7", icon: "st.switches.switch.off", backgroundColor: "#ffffff"
		}
		...
		details(["switch", "power", "energy","outlet1", "power1", "energy1", "outlet2", "power2", "energy2", "switch1", "power3", "energy3","switch2", "power4", "energy4","switch3", "power5", "energy5","switch4", "power6", "energy6", "Mode", "configure","reset","refresh"])
	}
}
...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicReport cmd) {
	...
	for ( i in 1..4 ) {
		...
		// log.debug "SW$i $sw"
		if(device.currentValue("switch$i")=="on") value="on$i"
	}
...
def zwaveEvent(physicalgraph.zwave.commands.switchbinaryv1.SwitchBinaryReport cmd) {
	...
	map = [name: "switch", value:value, type: "digital"]
	if(switchAll=="true") {
		for ( i in 1..4 ) { sendEvent(name: "switch$i", value:value$i, type: "digital") }
	}
	map
}
	...
	if (cmd?.scale == 0) {
		...
		if(endPoint < 3) { 
			...
			if(cmd.scaledMeterValue > outletWatts) {
				sendEvent( name: "outlet" + endPoint, value: "ono${endPoint}" )
				sendEvent( name: "switch" + (endPoint+4), value: "on${endPoint+4}" )
			} else {
				sendEvent( name: "outlet" + endPoint, value: "offo${endPoint}" )
				sendEvent( name: "switch" + (endPoint+4), value: "off${endPoint+4}" )
			}
		}
	}
...
def on1() { on(1) }; def off1() { off(1) }
def on2() { on(2) }; def off2() { off(2) }
def on3() { on(3) }; def off3() { off(3) }
def on4() { on(4) }; def off4() { off(4) }
...
def on7() {
	...
	sendEvent(name:"switch7", value:"on7")
}
...
def off7() {
	...
	delayBetween(cmds,300)
	sendEvent(name:"switch7", value:"off7")
}
